...
namespace lo
{
	...
	bool Constraint_Finder::add_adjacent_constraint(cloudblock_Ptrs &blocks, constraints &cons, int node_count)
	{
		...
		// *adjacent_con.block1 = blocks[node_count - 2];
		// *adjacent_con.block2 = blocks[node_count - 1]; //this and makeshared would not let the pointers share the same memory
	...
	Eigen::Matrix4d Constraint_Finder::double_check_tran(Eigen::Matrix4d &global_reg_tran, Eigen::Matrix4d &lo_predicted_tran,
...
// For the pose graph, use BFS to read the edge's point cloud and do the regsitration
