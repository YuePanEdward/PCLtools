...
int StereoBinaryFeature::hammingDistance(const StereoBinaryFeature &bsc1, const StereoBinaryFeature &bsc2)
{
	if (bsc1.size_ == bsc2.size_)
	{
		//XOR
		int one_count = 0;
		for (int i = 0; i < bsc1.byte_; i++)
		{
			one_count += byteBitsLookUp(bsc1.feature_[i] ^ bsc2.feature_[i]);
		}
		return one_count;
	}
	else
...
// bool StereoBinaryFeature::get_bsc_correspondence(doubleVectorSBF &bscs_1, doubleVectorSBF &bscs_2, std::vector<std::pair<int,int>> &corres)
// {
//    return true;
// }

//Output the binary feature
void StereoBinaryFeature::writeFeatures(const std::vector<StereoBinaryFeature> &features, const std::string &path)
{
