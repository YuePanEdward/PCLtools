...
namespace lo
{
                                      ...
                                      float dynamic_dist_thre_max, float near_dist_thre, bool recalculate_feature_on)
    {
        ...
        cf.dist_filter(last_target_cblock->pc_vertex, last_frame_reliable_radius);
        ...
        cf.random_downsample_pcl(local_map->pc_ground, kept_ground_num); //TODO: actually, we do not need so many ground points, we'd rather use unground points
        ...
        if (recalculate_feature_on) //Re-calculation primary vector and filter some of the points in the local map at the same time
        {
            ...
            float pca_radius = 1.5;
            int pca_max_k = 20;
            int pca_min_k = 6;
            float sin_high_pillar = 0.87; //60 deg
            float sin_low_beam = 0.25;    //15 deg

            //pillar
            if (used_feature_type[1] == '1')
                update_cloud_vectors(local_map->pc_pillar, local_map->tree_pillar, pca_radius, pca_max_k, pca_min_k, 0.0, sin_high_pillar);
            //beam
            if (used_feature_type[3] == '1')
                update_cloud_vectors(local_map->pc_beam, local_map->tree_beam, pca_radius, pca_max_k, pca_min_k, sin_low_beam, 1.0);
            ...
            LOG(INFO) << "Update pillar and beam feature points done in [" << time_update_local_map_linear_pts.count() * 1000.0 << "] ms.\n";
        }
                                                           ...
                                                           float dynamic_dist_thre_min, float dynamic_dist_thre_max, float near_dist_thre)
    {
        ...
        for (i = 0; i < feature_pts->points.size(); i++)
        {
            ...
            {
                map_kdtree->nearestKSearch(feature_pts->points[i], 1, search_indices, distances);                                    //search nearest neighbor
                if ((distances[0] > near_dist_thre && distances[0] < dynamic_dist_thre_min) || distances[0] > dynamic_dist_thre_max) // the distance should not be too close to keep the map more uniform
                    feature_pts_temp->points.push_back(feature_pts->points[i]);
    ...
    bool MapManager::update_cloud_vectors(pcTPtr feature_pts, const pcTreePtr map_kdtree,
                                          ...
                                          int k_min, float sin_low, float sin_high)
    {
        ...
        {
            if (pca_features[i].pt_num >= k_min)
            {
