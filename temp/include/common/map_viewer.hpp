...
// template <typename PointT>
// int MapViewer<PointT>::display_time_ms_;
                                                ...
                                                void *viewer_void)
{
    ...
    if (event.keyDown())
    {
        ...
        else if (event.getKeySym() == "F5")
        {
            if (!enabled_jump_)
            {
                std::cout << "[F5] was pressed => enable the jump to x frame function" << std::endl;
                enabled_jump_ = true;
            }
            else
            {
                std::cout << "[F5] was pressed => ban the jump to x frame function" << std::endl;
                enabled_jump_ = false;
            }
        }
        ...
        else if (event.getKeySym() == "F6")
        {
            if (!reset_camera_)
            {
                std::cout << "[F6] was pressed => reset camera" << std::endl;
                reset_camera_ = true;
            }
        }
        ...
        else if (event.getKeySym() == "h")
        {
            std::cout << "| Help:\n-------\n"
                      ...
                      << "          S      : turn on/off the visualization of distance (30,60,90m) circles\n"
                                                ...
                                                int display_time_ms)
{
    ...
    if (show_pose_graph_node_)
    {
        ...
        add_lines_to_viewer(viewer, pc_nodes_1, pc_nodes_2, "submao_bbx", edge_r, edge_g, edge_b, 0);

        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, line_width, "submao_bbx");
...
template <typename PointT>
void MapViewer<PointT>::update_pg_realtime(const constraint_t &current_con,
                                           boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer, int display_time_ms)
{
    float f_red, f_green, f_blue;
    float sphere_size = 30.0;

    char t[256];
    std::string s_e, s_v1, s_v2;
    sprintf(t, "edge_%d", current_con.unique_id);
    s_e = t;
    sprintf(t, "vertex1_%d", current_con.unique_id);
    s_v1 = t;
    sprintf(t, "vertex2_%d", current_con.unique_id);
    s_v2 = t;

    viewer->removeShape(s_e);
    viewer->removeShape(s_v1);
    viewer->removeShape(s_v2);

    switch (current_con.block1->data_type)
    {
    case ALS: //ALS
        f_red = 1.0;
        f_green = 0.0;
    ...
    pcl::PointXYZ ptc1;
    ptc1.x = current_con.block1->cp.x;
    ptc1.y = current_con.block1->cp.y;
    ptc1.z = 0;

    viewer->addSphere(ptc1, sphere_size, f_red, f_green, f_blue, s_v1);

    switch (current_con.block2->data_type)
    {
    case ALS: //ALS
        f_red = 1.0;
        f_green = 0.0;
    ...
    pcl::PointXYZ ptc2;
    ptc2.x = current_con.block2->cp.x;
    ptc2.y = current_con.block2->cp.y;
    ptc2.z = 0;

    viewer->addSphere(ptc2, sphere_size, f_red, f_green, f_blue, s_v2);

    switch (current_con.con_type)
    {
    ...
    viewer->addLine(ptc1, ptc2, f_red, f_green, f_blue, s_e);

    LOG(WARNING) << "Update pose graph: constriant [" << current_con.unique_id << "]";
                                            ...
                                            int display_time_ms, int display_downsample_ratio_current, int display_downsample_ratio_history)
{
    ...
    std::string instruction = "[Space]: pause/resume\t [T]: show/hide gt trajectory\t [G]: show/hide scale bar\t [F1-4]: switch color map (single, framewise, elevation, intensity)\n[F8] : show / hide local feature map\t[F9] : show / hide dense map \t[F10] : show / hide feature points in feature viewer \n[F11] : show / hide adjacent registration result in registration viewer \t[F12] : show / hide pose graph ";
    std::string map_plane_name = "map_plane";
    float sphere_size = 0.5;
    int font_size_frame = 18;
    ...
    if (is_seed_origin_)
    {
        ...
        viewer->addText(frame_name, 10, 70, font_size_frame, font_color, font_color, font_color, "frame_name");
    ...
    else
    {
        ...
        viewer->updateText(frame_name, 10, 70, font_size_frame, font_color, font_color, font_color, "frame_name");
    ...
    if (show_sparse_map_)
    {
        int sample_frame_rate = 2; //keep just one frame's downsampled point cloud in 3 frames
        ...
        case SINGLE: // Golden
        {
            ...
            for (size_t i = 0; i < pc_frame_visual_profile->points.size(); ++i)
            {
                ...
                apply_label_color_mapping((int)(pc_frame_visual_profile->points[i].curvature), r_c, g_c, b_c);
            ...
            if (current_frame->unique_id % sample_frame_rate == 0)
        ...
        viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1.5, frame_name);
        judge_pause(viewer, display_time_ms);
    }
    else
        judge_pause(viewer, 1);
...

{
    ...
    if (show_correspondences_)
    {
        ...
        *kpc_0_copy = *kpc_0;
        ...
        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 0.5, "corres");
...

{
    ...
    if (show_correspondences_)
    {
        ...
        //typename pcl::PointCloud<PointT>::Ptr keypoints_source_tran_copy(new pcl::PointCloud<PointT>());
        *keypoints_source_copy = *kpc_source;
        ...
        viewer->addPointCloud(cloud_target, "pointcloudT", v2);
        ...
        viewer->addPointCloud(keypoints_target, "keypointT", v2);
        ...
        for (int i = 0; i < kpc_source_tran->points.size(); ++i)
        {
            ...
            pt.g = 0;
            pt.b = 0;
            keypoints_source_copy->points.push_back(pt);
        } // Red
        viewer->addPointCloud(keypoints_source_copy, "keypointS_tran", v2);
        ...
        pcl::transformPointCloud(*keypoints_source, *keypoints_source_copy, pre_tranmat);

        add_lines_to_viewer(viewer, keypoints_source_copy, kpc_target, "corres", red, green, blue, v1);
        add_lines_to_viewer(viewer, kpc_source_tran, kpc_target, "corres_tran", red, green, blue, v2);

        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 0.5, "corres");
        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 0.5, "corres_tran");
