	...
	public:
				   ...
				   bool build_sample_pattern = false) : extract_radius_(extract_radius), voxel_side_num_(voxel_side_num)
		{
			...
			if (build_sample_pattern)
			{
				...
				std::ofstream fout("bsc_sample_pattern.txt");
			...
			{
				std::ifstream fin("bsc_sample_pattern.txt");
        ...
        /*main entrance */
		void extract_bsc_features(const typename pcl::PointCloud<PointT>::Ptr &input_cloud,
								  const typename pcl::PointCloud<PointT>::Ptr &key_points, int dof_type,
								  doubleVectorSBF &bscFeatures)
		{

			int feature_num;
			int min_neighbor_count = 6;

			if (key_points->points.size() > 0)
				feature_num = key_points->points.size();
			else
			{
				std::cout << "The input indice is NaN\n";
				return;
			}

			std::chrono::steady_clock::time_point tic_feature = std::chrono::steady_clock::now();
			//Initialization
			std::vector<StereoBinaryFeature> features_coor0(feature_num), features_coor1(feature_num), features_coor2(feature_num), features_coor3(feature_num);

			//LOG(INFO) << input_cloud->points.size() << " points found";

			//KD Tree indexing
			pcl::KdTreeFLANN<PointT> tree;
			tree.setInputCloud(input_cloud);

			std::chrono::steady_clock::time_point toc_build_tree = std::chrono::steady_clock::now();
			std::chrono::duration<double> time_build_tree = std::chrono::duration_cast<std::chrono::duration<double>>(toc_build_tree - tic_feature);
...
#pragma omp parallel for private(i) //Multi-thread
			...
			{
				std::vector<StereoBinaryFeature> features;

				std::vector<int> searchIndexes;
				std::vector<float> distances;

				tree.radiusSearch(key_points->points[i], sqrt(3.0) * extract_radius_, searchIndexes, distances);

				if (searchIndexes.size() < min_neighbor_count)
					continue; //features' indices would be -1 then

				extractBinaryFeatureOfKeypoint(input_cloud, key_points->points[i], searchIndexes, dof_type, features); // For each keypoint , extract the features

				features_coor0[i] = features[0];
				features_coor0[i].keypointIndex_ = i;

				//LOG(INFO)<<features_coor0[i].feature_;
				...
				std::vector<int>().swap(searchIndexes);
				std::vector<StereoBinaryFeature>().swap(features);
			}
			...
			std::vector<StereoBinaryFeature>().swap(features_coor0);
			std::vector<StereoBinaryFeature>().swap(features_coor1);
			std::vector<StereoBinaryFeature>().swap(features_coor2);
			std::vector<StereoBinaryFeature>().swap(features_coor3);

			std::chrono::steady_clock::time_point toc_feature = std::chrono::steady_clock::now();
			std::chrono::duration<double> time_feature = std::chrono::duration_cast<std::chrono::duration<double>>(toc_feature - tic_feature);

			LOG(INFO) << "Extract BSC feature done for [" << feature_num << "] points in [" << 1000.0 * time_feature.count() << "] ms";
		}
	 ...
	 as the X axis), push the points that has been transformed to the LCS into the result_cloud.*/
		bool computeLocalCoordinateSystem(const typename pcl::PointCloud<PointT>::Ptr &input_cloud, //input point cloud
										  PointT &key_pt,											// keypoint
										  const std::vector<int> &search_indices,					//indices of the keypoint's neighbor points
										  CoordinateSystem &localCoordinateSystem)					//LCS's origin and direction;
		{
			...
			computeEigenVectorsByWeightPCA(input_cloud, search_indices, key_pt, principalDirectionPca, middleDirectionPca, normalDirectionPca); //weighted PCA
			...
			localCoordinateSystem.origin.x() = key_pt.x;
			localCoordinateSystem.origin.y() = key_pt.y;
			localCoordinateSystem.origin.z() = key_pt.z;
											  ...
											  typename pcl::PointCloud<PointT>::Ptr &result_cloud)		//Output the rotation-variant point cloud under the LCS
		{
			...
			{
				PointT pt;
				pt.x = input_cloud->points[search_indices[i]].x - key_pt.x;
				pt.y = input_cloud->points[search_indices[i]].y - key_pt.y;
				pt.z = input_cloud->points[search_indices[i]].z - key_pt.z;
				//pt.intensity = input_cloud->points[search_indices[i]].intensity; //add the intensity feature ? TODO:
		...
		bool constructCubicGrid(const typename pcl::PointCloud<PointT>::Ptr &rotated_cloud, //the rotation-variant point cloud under the LCS
								std::vector<GridVoxel> &grid)								//grid result
		{
			//LOG(WARNING) << "pt_num:" << rotated_cloud->points.size();

			if (rotated_cloud->points.size() < 1)
				return false;
			...
			for (int i = 0; i < voxel_side_num_; i++)
			{
				for (int j = 0; j < voxel_side_num_; j++)
				{
					pcl::PointXY pt;
					...
					if (distances.empty() != true)
					{
						...
						for (int n = 0; n < search_indices.size(); n++)
						{
			...
			tree.reset(new pcl::KdTreeFLANN<pcl::PointXY>());
			//LOG(INFO)<< "xoy ok";
	    ...
	    for (size_t i = 0; i < rotated_cloud->size(); i++)
	{
		...
		bool ReArrangeGrid(std::vector<GridVoxel> &grid_a, std::vector<GridVoxel> &grid_b, int tran_xy, int tran_xz, int tran_yz)
		{
			int size_vector = grid_a.size();
			int size_one_plane = voxel_side_num_ * voxel_side_num_;

			std::vector<GridVoxel> grid_a_xy(grid_a.begin(), grid_a.begin() + size_one_plane);
			std::vector<GridVoxel> grid_a_xz(grid_a.begin() + size_one_plane, grid_a.begin() + 2 * size_one_plane);
			std::vector<GridVoxel> grid_a_yz(grid_a.begin() + 2 * size_one_plane, grid_a.begin() + 3 * size_one_plane);

			std::vector<GridVoxel> grid_b_xy, grid_b_xz, grid_b_yz;
		...
		//1
		bool ReArrange_reverse_all(std::vector<GridVoxel> &grid_2d_a, std::vector<GridVoxel> &grid_2d_b)
		{
		...
		//2
		bool ReArrange_reverse_sym_2(std::vector<GridVoxel> &grid_2d_a, std::vector<GridVoxel> &grid_2d_b)
		{
			grid_2d_b.resize(grid_2d_a.size());
			int i, j, original_k;
			for (int k = 0; k < grid_2d_a.size(); k++)
		...
		//3
		bool ReArrange_reverse_sym_1(std::vector<GridVoxel> &grid_2d_a, std::vector<GridVoxel> &grid_2d_b)
		{
			grid_2d_b.resize(grid_2d_a.size());
			int i, j, original_k;
			for (int k = 0; k < grid_2d_a.size(); k++)
		...
		bool ReArrange_2D(std::vector<GridVoxel> &grid_2d_a, std::vector<GridVoxel> &grid_2d_b, int tran_type)
		{
			switch (tran_type)
			{
			case 1:
		...
		bool extractBinaryFeatureOfKeypoint(const typename pcl::PointCloud<PointT>::Ptr &input_cloud,
											PointT &key_pt, const std::vector<int> &search_indexes, int dof_type,
											vectorSBF &features)
		{
			...
			computeLocalCoordinateSystem(input_cloud, key_pt, search_indexes, localSystem1);
			transformPointCloudToLocalSystem(input_cloud, key_pt, search_indexes, localSystem1, result_cloud); //transform to LRCS

			std::vector<GridVoxel> grid_1(gridFeatureDimension_);
			...
			if (dof_type > 0)
			{
				...
				std::vector<GridVoxel> grid_2(gridFeatureDimension_);
				...
				std::vector<GridVoxel>(gridFeatureDimension_).swap(grid_2);

				if (dof_type > 4) //(new coordiante system used for 6DOF matching), 4 features/CS in total
				{
					std::vector<GridVoxel> grid_3(gridFeatureDimension_);
					ReArrangeGrid(grid_1, grid_3, 3, 3, 1); //TODO: check
					...
					std::vector<GridVoxel> grid_4(gridFeatureDimension_);
					...
					std::vector<GridVoxel>(gridFeatureDimension_).swap(grid_3);
					std::vector<GridVoxel>(gridFeatureDimension_).swap(grid_4);
				}
			}

			//Clear grid and result cloud
			std::vector<GridVoxel>(gridFeatureDimension_).swap(grid_1);
	...
	private:
		...
		//Do weighted 3D PCA (you may use this pca), which might be better
		...
		{
			//you may directly use general pca
			...
			double center_x(0.0), center_y(0.0), center_z(0.0), dis_all(0.0);
			for (int it = 0; it < search_indices.size(); ++it)
			{
				center_x += input_cloud->points[search_indices[it]].x;
				center_y += input_cloud->points[search_indices[it]].y;
				center_z += input_cloud->points[search_indices[it]].z;

				dis_all += (radius - Comput3DDistanceBetweenPoints(input_cloud->points[search_indices[it]], key_pt)); //TODO: delete the redundant codes and speed up
			}
			center_x /= search_indices.size();
			center_y /= search_indices.size();
			center_z /= search_indices.size();
			...
			{
				float dis, weight;
				dis = Comput3DDistanceBetweenPoints(input_cloud->points[search_indices[it]], key_pt);
				weight = radius - dis;
				covariance(0, 0) += weight * (input_cloud->points[search_indices[it]].x - center_x) * (input_cloud->points[search_indices[it]].x - center_x);
				covariance(0, 1) += weight * (input_cloud->points[search_indices[it]].x - center_x) * (input_cloud->points[search_indices[it]].y - center_y);
