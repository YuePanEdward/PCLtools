...
#pragma omp parallel for private(i) //Multi-thread
	...
	void apply_motion_compensation(typename pcl::PointCloud<PointT>::Ptr pc_in_out, Eigen::Matrix4d &Tran, float s_ambigous_thre = 0.000)
	{
		...
		{
			if (pc_in_out->points[i].curvature < s_ambigous_thre || pc_in_out->points[i].curvature > 1.0 - s_ambigous_thre)
				continue;
			...
			Eigen::Vector3d undistort_point = d_quat * temp_point + d_translation;
			PointT pt = pc_in_out->points[i];
			pt.x = undistort_point(0);
			pt.y = undistort_point(1);
			pt.z = undistort_point(2);
			cloud_temp->points.push_back(pt);
		}
		cloud_temp->points.swap(pc_in_out->points);
	}
								   ...
								   Eigen::Matrix4d &Tran, float s_ambigous_thre = 0.0)
	{
		*pc_out = *pc_in;
		...
		{
			if (pc_in->points[i].curvature < s_ambigous_thre || pc_in->points[i].curvature > 1.0 - s_ambigous_thre)
				continue;
	...
	bool prune_unstable_pts(const typename pcl::PointCloud<PointT>::Ptr &cloud_in, typename pcl::PointCloud<PointT>::Ptr &cloud_out,
							const std::vector<pca_feature_t> &features, float stable_ratio_max, float min_curvature = 0.0,
	...
	bool non_max_suppress(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
						  typename pcl::PointCloud<PointT>::Ptr &cloud_out, float non_max_radius,
						  bool kd_tree_already_built = false, const typename pcl::search::KdTree<PointT>::Ptr &built_tree = NULL) //according to curvature
	{
		std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
	...
	bool non_max_suppress(std::vector<pca_feature_t> &features, pcl::PointIndicesPtr &indices, float curvature_non_max_radius)
	{
							...
							bool apply_grid_wise_outlier_filter = false, float outlier_std_scale = 3.0) //current intensity_thre is for kitti dataset (TODO: disable it)
	{
		...
		//eliable_neighbor_grid_num_thre = 2;
		...
		for (int j = 0; j < cloud_in->points.size(); j++)
		{
			...
			if (temp_id >= 0 && temp_id < num_grid)
			{
				...
				{
					distance_weight = 1.0 * standard_distance / grid[temp_id].dist2station;
		...
		double consuming_time_ransac = 0;

		//TODO: Use omp here (note that std::vector and push_back may cast some problem because they are not thread-safe)
		//For each grid
		for (int i = 0; i < num_grid; i++)
		{
			//Filtering some grids with too little points
			...
			{
				distance_weight = 1.0 * standard_distance / grid[i].dist2station;
				if (distance_weight_downsampling_method == 1) //linear weight
				{
				...
				if (estimate_ground_normal_method == 3 && grid_ground->points.size() >= min_grid_pt_num)
				{
					...
					//r is the inlier ratio (> 0.5 in our case), N is 3 in our case (3 points can fit a plane), to get a confidence > 0.999, we need about 40 iteration (M=40)
					estimate_ground_normal_by_ransac(grid_ground, 0.3 * max_height_difference, 35, normal_x, normal_y, normal_z);

					for (int j = 0; j < grid_ground->points.size(); j++)
					{
						if (j % ground_random_down_rate_temp == 0 && std::abs(normal_z) > 0.85) //70 deg
						{
		...
		std::chrono::duration<double> ground_seg_time = std::chrono::duration_cast<std::chrono::duration<double>>(toc_1 - tic);
		std::chrono::duration<double> ground_normal_time = std::chrono::duration_cast<std::chrono::duration<double>>(toc_2 - toc_1);
	...
	bool classify_nground_pts(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
							  ...
							  bool fixed_num_downsampling = false, int pillar_down_fixed_num = 200, int facde_down_fixed_num = 500, int beam_down_fixed_num = 200,
							  int roof_down_fixed_num = 0, int unground_down_fixed_num = 20000,
							  float beam_height_max = FLT_MAX, float roof_height_min = -FLT_MAX, bool sharpen_with_nms = true)

	{

		std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();

		// beam_height_max = 0.3; //TODO:
		// roof_height_min=0.0;

		if (fixed_num_downsampling)
			random_downsample_pcl(cloud_in, unground_down_fixed_num);

		std::vector<float> timestamp_array; //save the timestamp for later processing
		for (int i = 0; i < cloud_in->points.size(); i++)
			timestamp_array.push_back(cloud_in->points[i].curvature);
		...
		pca_estimator.get_pc_pca_feature(cloud_in, cloud_features, tree, neighbor_searching_radius, neighbor_k, 1, pca_down_rate, true, 35.0);
		...
		if (curvature_thre < 0.001) // set stablilty_thre as 0 to disable the vertex extraction
		...
		prune_unstable_pts(cloud_in, cloud_vertex, cloud_features, 3.0 * curvature_thre, 0.25 * curvature_thre, neigh_k_min);
		...
		{
			float vertex_feature_ratio_thre = 0.4 / pca_down_rate;
			for (int i = 0; i < cloud_in->points.size(); i++)
			{
		...
		if (sharpen_with_nms)
		{
			non_max_suppress(cloud_pillar, cloud_pillar_down, 0.5 * neighbor_searching_radius);
			non_max_suppress(cloud_beam, cloud_beam_down, 0.5 * neighbor_searching_radius);
			non_max_suppress(cloud_facade, cloud_facade_down, 0.5 * neighbor_searching_radius);
			non_max_suppress(cloud_roof, cloud_roof_down, 0.5 * neighbor_searching_radius);
		}
		...
		if (fixed_num_downsampling)
		{
			...
			int sector_num = 4;
			xy_normal_balanced_downsample(cloud_facade_down, (int)(facde_down_fixed_num / sector_num), sector_num);
			//random_downsample_pcl(cloud_facade_down, 1000);
		...
		std::chrono::steady_clock::time_point toc_3 = std::chrono::steady_clock::now();
		std::chrono::duration<double> time_used = std::chrono::duration_cast<std::chrono::duration<double>>(toc_3 - tic);
		//std::chrono::duration<double> time_used_vertex = std::chrono::duration_cast<std::chrono::duration<double>>(toc_2 - toc_1);

		//Free the memory
		std::vector<pca_feature_t>().swap(cloud_features);
		std::vector<int>().swap(index_with_feature);

		LOG(INFO) << "Feature points extracted done in [" << time_used.count() * 1000.0 << "] ms.";
	...
	bool extract_semantic_pts(cloudblock_Ptr in_block,
							  ...
							  bool use_adpative_parameters = false, bool apply_scanner_filter = true,
							  ...
							  int gf_down_down_rate_ground = 2, int pca_neighbor_k_min = 8, float intensity_thre = FLT_MAX, //default intensity_thre means that highly-reflective objects would not be prefered
							  ...
							  bool sharpen_with_nms_on = true, bool fixed_num_downsampling = false, int ground_down_fixed_num = 0,
							  int pillar_down_fixed_num = 0, int facade_down_fixed_num = 0, int beam_down_fixed_num = 0,
							  int roof_down_fixed_num = 0, int unground_down_fixed_num = 0, float beam_height_max = FLT_MAX, float roof_height_min = -FLT_MAX,
							  float approx_scanner_height = 2.0, float underground_thre = -7.0,
							  bool semantic_assisted = false, bool apply_roi_filtering = false, float roi_min_y = 0.0, float roi_max_y = 0.0)
	{
		...
		//double std_sor_filter = 2.0;
		//sor_filter(in_block->pc_down, in_block->pc_down, K_for_cal, std_sor_filter);
		...
		classify_nground_pts(in_block->pc_unground, in_block->pc_pillar, in_block->pc_beam,
							 ...
							 pca_neighbor_radius, pca_neighbor_k, pca_neighbor_k_min,
							 ...
							 beam_height_max, roof_height_min, sharpen_with_nms_on);
	...
	//Used in main CCN
	bool extract_semantic_pts(cloudblock_Ptr in_block, bool clear_raw_cloud,
							  float vf_down_res_als, float vf_down_res_tls,
							  float vf_down_res_mls, float vf_down_res_bpls,
							  float gf_grid_resolution, float gf_max_grid_height_diff, float gf_neighbor_height_diff,
							  int gf_down_rate_ground, int gf_downsample_rate_nonground,
							  float pca_neighbor_radius, int pca_neighbor_k,
							  float edge_thre, float planar_thre, float curvature_thre,
							  int estimate_ground_normal_method = 1,	//estimate_ground_normal_method, 0: directly use (0,0,1), 1: estimate normal in neighborhood, 2: use ransac to estimate plane coeffs in a grid
							  float normal_estimation_radius = 2.0,		//only when enabled when estimate_ground_normal_method = 1
							  int distance_inverse_sampling_method = 0, //distance_inverse_downsample
							  float standard_distance = 25.0,			//the distance where the weight is 1, only useful when distance_weight_downsample is on
							  int extract_vertex_points_method = 1,		//use the maximum curvature based keypoints
							  int gf_grid_pt_num_thre = 12, float gf_max_ground_height = 8.0,
							  int gf_down_down_rate_ground = 2, int gf_reliable_neighbor_grid_thre = 0, int pca_neighbor_k_min = 8,
							  float linear_vertical_sin_high_thre = 0.87, float linear_vertical_sin_low_thre = 0.5,
							  float planar_vertical_sin_high_thre = 0.87, float planar_vertical_sin_low_thre = 0.5) //60 degree, 30 degree
	{
		voxel_downsample_4type(in_block, vf_down_res_als, vf_down_res_tls, vf_down_res_mls, vf_down_res_bpls);

		random_downsample(in_block->pc_down, in_block->pc_sketch, in_block->pc_down->points.size() / 2048 + 1);

		//disabled for main ccn
		// if (in_block->is_single_scanline) // multi-scan line (station centric coordinate system)
		// 	distance_inverse_downsample_method = 0;

		fast_ground_filter(in_block->pc_down, in_block->pc_ground, in_block->pc_ground_down, in_block->pc_unground, in_block->pc_vertex,
						   gf_grid_pt_num_thre, gf_grid_resolution, gf_max_grid_height_diff, gf_neighbor_height_diff,
						   gf_max_ground_height, gf_down_rate_ground, gf_down_down_rate_ground,
						   gf_downsample_rate_nonground, gf_reliable_neighbor_grid_thre, estimate_ground_normal_method, normal_estimation_radius,
						   distance_inverse_sampling_method, standard_distance);

		float edge_thre_down = edge_thre + 0.1;
		float planar_thre_down = planar_thre + 0.1;
		float vertex_curvature_non_max_r = 3.0 * vf_down_res_als;

		classify_nground_pts(in_block->pc_unground, in_block->pc_pillar, in_block->pc_beam, in_block->pc_facade,
							 in_block->pc_roof, in_block->pc_pillar_down, in_block->pc_beam_down,
							 in_block->pc_facade_down, in_block->pc_roof_down, in_block->pc_vertex,
							 pca_neighbor_radius, pca_neighbor_k, pca_neighbor_k_min,
							 edge_thre, planar_thre, edge_thre_down, planar_thre_down,
							 extract_vertex_points_method, curvature_thre, vertex_curvature_non_max_r,
							 linear_vertical_sin_high_thre, linear_vertical_sin_low_thre,
		...
		in_block->down_feature_point_num = in_block->pc_ground_down->points.size() + in_block->pc_pillar_down->points.size() + in_block->pc_beam_down->points.size() + in_block->pc_facade_down->points.size() + in_block->pc_roof_down->points.size() + in_block->pc_vertex->points.size();

		if (clear_raw_cloud)
		{
			in_block->free_raw_cloud();
			LOG(INFO) << "Raw point cloud memory released";
		}
	...
	bool voxel_downsample_4type(cloudblock_Ptr in_block, float vf_down_res_als,
								float vf_down_res_tls, float vf_down_res_mls, float vf_down_res_bpls)
	{
		float vf_downsample_resolution = 0.1;

		switch (in_block->data_type)
		{
		case ALS: //ALS
			vf_downsample_resolution = vf_down_res_als;
			break;
		case TLS: //TLS
			vf_downsample_resolution = vf_down_res_tls;
			break;
		case MLS: //MLS
			vf_downsample_resolution = vf_down_res_mls;
			break;
		case BPLS: //BPLS
			vf_downsample_resolution = vf_down_res_bpls;
			break;
		default:
			break;
		}

		if (voxel_downsample(in_block->pc_raw, in_block->pc_down, vf_downsample_resolution))
			return 1;
		else
			return 0;
	}

	bool batch_down_pc_pair(const constraint_t &this_con,
							...
							float ALS_radius, float TLS_radius, float MLS_radius, float BPLS_radius)
	{
		...
		{
		case ALS: //ALS
			voxel_downsample(cloud1, subcloud1, ALS_radius);
			break;
		case TLS: //TLS
			voxel_downsample(cloud1, subcloud1, TLS_radius);
			break;
		case MLS: //MLS
			voxel_downsample(cloud1, subcloud1, MLS_radius);
			break;
		case BPLS: //BPLS
			voxel_downsample(cloud1, subcloud1, BPLS_radius);
			break;
		default:
			break;
		}
		...
		{
		case ALS: //ALS
			voxel_downsample(cloud2, subcloud2, ALS_radius);
			break;
		case TLS: //TLS
			voxel_downsample(cloud2, subcloud2, TLS_radius);
			break;
		case MLS: //MLS
			voxel_downsample(cloud2, subcloud2, MLS_radius);
			break;
		case BPLS: //BPLS
			voxel_downsample(cloud2, subcloud2, BPLS_radius);
			break;
		default:
			break;
		}
