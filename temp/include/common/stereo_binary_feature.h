...
namespace lo
{
	...
	public:
		...
		std::vector<int> bscVisualWordsIndexV_;

		int keypointIndex_ = -1;
		...
		StereoBinaryFeature &operator=(const StereoBinaryFeature &sbf)
		{
			...
			if (size_ > 0)
			{
				...
				this->keypointIndex_ = sbf.keypointIndex_;
		...
		//bool get_bsc_correspondence(doubleVectorSBF &bscs_1, doubleVectorSBF &bscs_2, std::vector<std::pair<int,int>> &corres);
		...
		void readFeatures(std::vector<StereoBinaryFeature> &features, const std::string &path);

		/*Write the binary feature*/
		void writeFeatures(const std::vector<StereoBinaryFeature> &features, const std::string &path);
	...
	typedef std::vector<SBF> vectorSBF;
	typedef std::vector<vectorSBF> doubleVectorSBF;
	typedef std::vector<doubleVectorSBF> TribleVectorSBF;
