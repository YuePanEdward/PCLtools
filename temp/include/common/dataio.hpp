...
namespace lo
{
    ...
    inline bool exists_file(const std::string &filename)
    {
        ...
        bool batch_read_filenames_in_folder(const std::string &folderName, const std::string &extension, std::vector<std::string> &fileNames,
        ...
        bool batch_read_filenames_in_folder(const std::string &folderName, const std::string &file_list_extenstion, const std::string &extension,
                                            std::vector<std::string> &fileNames, int frame_begin = 0, int frame_end = 99999, int frame_step = 1)
        {
        ...
        bool batch_read_filenames_in_folder(const std::string &folderName, const std::string &file_list_extenstion, const std::string &extension, std::vector<std::string> &fileNames,
                                            bool &station_position_available, bool &station_pose_available, bool &is_single_scanline)
        {
            std::string filename_list = folderName + file_list_extenstion;
            ...
            std::string str;
            name_list_file >> str >> station_position_available;
            name_list_file >> str >> station_pose_available;
            name_list_file >> str >> is_single_scanline;
            name_list_file >> str;
        ...
        bool batch_read_filenames_in_folder_subfolder(const std::string &folderName, const std::string &extension, std::vector<std::string> &fileNames)
        {
                          ...
                          std::vector<std::vector<double>> &coordinatesUTM_XYZ)
    {
        ...
        bool read_pc_cloud_block(cloudblock_Ptr &in_block)
        {
            if (read_cloud_file(in_block->filename, in_block->pc_raw))
            {
                ...
                // LOG(INFO)<<"Get local bound and cp";
                // LOG(INFO)<<"("<<in_block->local_bound.min_x<<","<<in_block->local_bound.min_y<<","<<in_block->local_bound.min_z<<")";
                // LOG(INFO)<<"("<<in_block->local_bound.max_x<<","<<in_block->local_bound.max_y<<","<<in_block->local_bound.max_z<<")";
                return 1;
            }
            else
                return 0;
        ...
        bool write_lo_pose_overwrite(Eigen::Matrix4d &Trans1_2, std::string &output_file)
        {
            ...
            out << setprecision(8);
            out << Trans1_2(0, 0) << "\t" << Trans1_2(0, 1) << "\t" << Trans1_2(0, 2) << "\t" << Trans1_2(0, 3) << "\t"
                << Trans1_2(1, 0) << "\t" << Trans1_2(1, 1) << "\t" << Trans1_2(1, 2) << "\t" << Trans1_2(1, 3) << "\t"
                << Trans1_2(2, 0) << "\t" << Trans1_2(2, 1) << "\t" << Trans1_2(2, 2) << "\t" << Trans1_2(2, 3) << "\n";
...
#if 0
        calib_mat(0, 0) = 4.276802385584e-04;
        calib_mat(0, 1) = -9.999672484946e-01;
        calib_mat(0, 2) = -8.084491683471e-03;
        calib_mat(0, 3) = -1.198459927713e-02;
        calib_mat(1, 0) = -7.210626507497e-03;
        calib_mat(1, 1) = 8.081198471645e-03;
        calib_mat(1, 2) = -9.999413164504e-01;
        calib_mat(1, 3) = -5.403984729748e-02;
        calib_mat(2, 0) = 9.999738645903e-01;
        calib_mat(2, 1) = 4.859485810390e-04;
        calib_mat(2, 2) = -7.206933692422e-03;
        calib_mat(2, 3) = -2.921968648686e-01;
#endif
        ...
        Matrix4ds load_poses_from_transform_matrix(std::string filepath, int frame_begin = 0, int frame_step = 1)
        {
            double tmp[12];
            Matrix4ds pose;
            ...
            while (posereader >> tmp[0])
            {
                ...
                if ((count >= frame_begin) && ((count - frame_begin) % frame_step == 0))
