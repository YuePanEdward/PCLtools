...
class MapViewer
{
  public:
    ...
    MapViewer(float initial_background_color, bool map_window_on = true,
              ...
              int initial_downsample_ratio = 1)
    {
    ...
    void set_show_reg(bool show_reg = 1) //default -> show
    {
        show_reg_ = show_reg;
    }

    void set_show_feature(bool show_feature = 1) //default -> show
    {
        show_feature_ = show_feature;
    }
    ...
    void update_pg_realtime(const constraint_t &current_con,
                            boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer,
                            int display_time_ms = 100);
    ...
    void display_correspondences_compare(boost::shared_ptr<pcl::visualization::PCLVisualizer> &viewer,
                                         const typename pcl::PointCloud<PointT>::Ptr &kpc_source, const typename pcl::PointCloud<PointT>::Ptr &kpc_target, const typename pcl::PointCloud<PointT>::Ptr &kpc_source_tran,
                                         const typename pcl::PointCloud<PointT>::Ptr &pc_source, const typename pcl::PointCloud<PointT>::Ptr &pc_target, const typename pcl::PointCloud<PointT>::Ptr &pc_source_tran,
                                         Eigen::Matrix4d &pre_tran, int display_time_ms = 50);
