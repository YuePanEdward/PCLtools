...
// Author: Yue Pan @ ETH Zurich & HESAI
//
...
namespace lo
{
	...
	{
	public:
		//General implement of icp registration algorithm in pcl
...
#pragma omp parallel for private(i) //Multi-thread
		...
		std:
			vector<std::pair<int, int>> dist_array;
...
#pragma omp parallel for private(i) //Multi-thread
		...
		bool coarse_reg_teaser(const typename pcl::PointCloud<PointT>::Ptr &target_pts,
							   const typename pcl::PointCloud<PointT>::Ptr &source_pts,
							   Eigen::Matrix4d &tran_mat, float noise_bound = 0.05)
		{
#if TEASER_ON
		...
		int pcl_icp(constraint_t &registration_cons,
					...
					bool launch_real_time_corr_viewer = false, bool launch_real_time_reg_viewer = false)
		{
		...
		//--------------------------------------------------------------------------------------------------------------------------//
		//Multiple Metrics Linear Least Square ICP (MMLLS-ICP)
		...
		//  This is sloved very efficiently by Linear Least Square (take tx,ty,tz,row,pitch,yaw as unknowns)
		...
		int mm_lls_icp(constraint_t &registration_cons, // cblock_1 (target point cloud), cblock_2 (source point cloud)
					   ...
					   bool apply_intersection_filter = true, bool apply_motion_undistortion_while_registration = false, bool use_more_points = false,
					   bool keep_less_source_points = false, float max_bearable_rotation_d = 45.0)
...
#pragma omp section
			...
			for (int i = 0; i < max_iter_num; i++)
			{
				...
				if (used_feature_type[0] == '1' && pc_ground_sc->size() > 0)
					determine_corres(pc_ground_sc, pc_ground_tc, registration_cons.block1->tree_ground, dis_thre_ground, corrs_ground, 1); //Check normal vector
				if (used_feature_type[1] == '1' && pc_pillar_sc->size() > 0)
					determine_corres(pc_pillar_sc, pc_pillar_tc, registration_cons.block1->tree_pillar, dis_thre_pillar, corrs_pillar, 1); //Check primary vector
				if (used_feature_type[2] == '1' && pc_facade_sc->size() > 0)
					determine_corres(pc_facade_sc, pc_facade_tc, registration_cons.block1->tree_facade, dis_thre_facade, corrs_facade, 1); //Check normal vector
				if (used_feature_type[3] == '1' && pc_beam_sc->size() > 0)
					determine_corres(pc_beam_sc, pc_beam_tc, registration_cons.block1->tree_beam, dis_thre_beam, corrs_beam, 1); //Check primary vector
				if (used_feature_type[4] == '1' && pc_roof_sc->size() > 0)
					determine_corres(pc_roof_sc, pc_roof_tc, registration_cons.block1->tree_roof, dis_thre_roof, corrs_roof, 1); //Check normal vector
				if (used_feature_type[5] == '1' && pc_vertex_sc->size() > 0)
					determine_corres(pc_vertex_sc, pc_vertex_tc, registration_cons.block1->tree_vertex, dis_thre_vertex, corrs_vertex, 0);
				...
				if (total_corr_num < min_total_corr_num || neccessary_corr_num < min_neccessary_corr_num)
				{
					process_code = -2;
				...
				//0: equal weight
				//1: x,y,z balanced weight
				//2: distance weight (based on 1)
				//3: residual weight (based on 1)
				//4: distance * residual weight (based on 1)
				...
				if (i == max_iter_num - 1 || (i > 2 && ts.norm() < converge_translation && std::abs(rs.angle()) < converge_rotation))
				{
					...
					//Calculate converged residual and information matrix //TODO: add weight
					...
					//information_matrix = (Dxx) ^(-1) =  Qxx * (sigma_post)^2 = ATPA / (sigma_post)^2  = (n-t)/(VTPV)*(ATPA)

					information_matrix = (1.0 / sigma_square_post) * cofactor_matrix.inverse(); //(the former one is ATPA)

					LOG(INFO) << "information_matrix:\n"
							  << information_matrix;
					...
					//TODO fix , use euler angle vc_matrix to show std
			...
			registration_cons.sigma = std::sqrt(sigma_square_post);	   //Final unit weight standard deviation (important)
								 ...
								 Eigen::Matrix4d initial_guess = Eigen::Matrix4d::Identity(), bool keep_less_source_points = false, float max_bearable_rotation_d = 10.0)
		{
			...
			*pc_ground_tc = *registration_cons.block1->pc_ground;
			...
			for (int i = 0; i < max_iter_num; i++)
			{
				...
				// First, find coorespondences
				// We need to check the normal compatibility of the plane correspondences
				determine_corres(pc_ground_sc, pc_ground_tc, tree_ground_tc, dis_thre_ground, corrs_ground_f, 1); //Check normal
	...
	private:
		...
		bool determine_corres(typename pcl::PointCloud<PointT>::Ptr &Source_Cloud, typename pcl::PointCloud<PointT>::Ptr &Target_Cloud,
							  ...
							  boost::shared_ptr<pcl::Correspondences> &Corr_f, bool normal_check,
							  bool duplicate_check = true, float angle_thre_degree = 35, int K_filter_distant_point = 800)
		...
		{
			float sin_half_row, cos_half_row, sin_half_pitch, cos_half_pitch, sin_half_yaw, cos_half_yaw;

			sin_half_row = sin(0.5 * euler_angle(0));
			sin_half_pitch = sin(0.5 * euler_angle(1));
			sin_half_yaw = sin(0.5 * euler_angle(2));
			cos_half_row = cos(0.5 * euler_angle(0));
			cos_half_pitch = cos(0.5 * euler_angle(1));
			cos_half_yaw = cos(0.5 * euler_angle(2));

			Jacobi(0, 0) = 0.5 * (-sin_half_row * cos_half_pitch * sin_half_yaw - cos_half_row * sin_half_pitch * cos_half_yaw);
			Jacobi(0, 1) = 0.5 * (-cos_half_row * sin_half_pitch * sin_half_yaw - sin_half_row * cos_half_pitch * cos_half_yaw);
			Jacobi(0, 2) = 0.5 * (cos_half_row * cos_half_pitch * cos_half_yaw + sin_half_row * sin_half_pitch * sin_half_yaw);

			Jacobi(1, 0) = 0.5 * (cos_half_row * cos_half_pitch * sin_half_yaw - sin_half_row * sin_half_pitch * cos_half_yaw);
			Jacobi(1, 1) = 0.5 * (-sin_half_row * sin_half_pitch * sin_half_yaw + cos_half_row * cos_half_pitch * cos_half_yaw);
			Jacobi(1, 2) = 0.5 * (sin_half_row * cos_half_pitch * cos_half_yaw - cos_half_row * sin_half_pitch * sin_half_yaw);

			Jacobi(2, 0) = 0.5 * (cos_half_row * cos_half_pitch * cos_half_yaw - sin_half_row * sin_half_pitch * sin_half_yaw);
			Jacobi(2, 1) = 0.5 * (-sin_half_row * sin_half_pitch * cos_half_yaw + cos_half_row * cos_half_pitch * sin_half_yaw);
			Jacobi(2, 2) = 0.5 * (-sin_half_row * cos_half_pitch * sin_half_yaw + cos_half_row * sin_half_pitch * cos_half_yaw);
		...
		bool multi_metrics_lls_tran_estimation(const typename pcl::PointCloud<PointT>::Ptr &Source_Ground, const typename pcl::PointCloud<PointT>::Ptr &Target_Ground, boost::shared_ptr<pcl::Correspondences> &Corr_Ground,
											   ...
											   Vector6d &unknown_x, Matrix6d &cofactor_matrix, int iter_idx, std::string weight_strategy, float z_xy_balance_ratio = 1.0,
											   float pt2pt_residual_window = 0.1, float pt2pl_residual_window = 0.1, float pt2li_residual_window = 0.1)
		{
			Matrix6d ATPA;
			Vector6d ATPb;
			...
			if (weight_strategy[0] == '1') //x,y,z balanced weighting
			{
				...
				w_ground = std::sqrt(max_(0.01, w_ground_square));
				w_roof = w_ground;
				w_facade = 1.0;
				w_pillar = 1.0;
				w_beam = 1.0;
			...
			//LOG(INFO) << "ground_weight = " << w_ground;

			bool dist_weight = false;
			bool residual_weight = false;
			bool intensity_weight = false;
			int iter_thre = 1; //the residual based weighting would only be applied after this number of iteration

			if (weight_strategy[1] == '1' && iter_idx > iter_thre) //weight according to residual //
			...
			pt2pl_lls_summation(Source_Ground, Target_Ground, Corr_Ground, ATPA, ATPb, w_ground, dist_weight, residual_weight, intensity_weight, pt2pl_residual_window);

			pt2pl_lls_summation(Source_Facade, Target_Facade, Corr_Facade, ATPA, ATPb, w_facade, dist_weight, residual_weight, intensity_weight, pt2pl_residual_window);
			pt2pl_lls_summation(Source_Roof, Target_Roof, Corr_Roof, ATPA, ATPb, w_roof, dist_weight, residual_weight, intensity_weight, pt2pl_residual_window);

			pt2li_lls_pri_direction_summation(Source_Pillar, Target_Pillar, Corr_Pillar, ATPA, ATPb, w_pillar, dist_weight, residual_weight, intensity_weight, pt2li_residual_window);
			pt2li_lls_pri_direction_summation(Source_Beam, Target_Beam, Corr_Beam, ATPA, ATPb, w_beam, dist_weight, residual_weight, intensity_weight, pt2li_residual_window);

			// pt2li_lls_summation(Source_Pillar, Target_Pillar, Corr_Pillar, ATPA, ATPb, w_pillar, dist_weight, residual_weight, intensity_weight, pt2li_residual_window);
			// pt2li_lls_summation(Source_Beam, Target_Beam, Corr_Beam, ATPA, ATPb, w_beam, dist_weight, residual_weight, intensity_weight, pt2li_residual_window);

			pt2pt_lls_summation(Source_Vertex, Target_Vertex, Corr_Vertex, ATPA, ATPb, w_vertex, dist_weight, residual_weight, intensity_weight, pt2pt_residual_window);
		...
		bool ground_3dof_lls_tran_estimation(const typename pcl::PointCloud<PointT>::Ptr &Source_Ground,
											 ...
											 int iter_idx, std::string weight_strategy)
		{
			Eigen::Matrix3d ATPA;
			Eigen::Vector3d ATPb;
			ATPA.setZero();
			...
			pt2pl_ground_3dof_lls_summation(Source_Ground, Target_Ground, Corr_Ground, ATPA, ATPb, 1.0, dist_weight, residual_weight, intensity_weight);
		...
		bool pt2pt_lls_summation(const typename pcl::PointCloud<PointT>::Ptr &Source_Cloud, const typename pcl::PointCloud<PointT>::Ptr &Target_Cloud,
								 boost::shared_ptr<pcl::Correspondences> &Corr, Matrix6d &ATPA, Vector6d &ATPb,
			...
			for (int i = 0; i < (*Corr).size(); i++)
			{
				...
				if (t_index != -1)
				{
					...
					if (dist_weight_or_not)
						wx = wx * get_weight_by_dist(std::sqrt(qx * qx + qy * qy));
								 ...
								 float residual_window_size = 0.1)
		{
			...
			for (int i = 0; i < (*Corr).size(); i++)
			{
				...
				if (t_index != -1)
				{
					...
					float dist = std::sqrt(qx * qx + qy * qy);

					if (dist_weight_or_not)
						w = w * get_weight_by_dist(dist);

					if (residual_weight_or_not)
						w = w * get_weight_by_residual(std::abs(d), residual_window_size /*std::sqrt(dist/15.0)*/);
		...
		bool pt2pl_ground_3dof_lls_summation(const typename pcl::PointCloud<PointT>::Ptr &Source_Cloud, const typename pcl::PointCloud<PointT>::Ptr &Target_Cloud,
											 boost::shared_ptr<pcl::Correspondences> &Corr, Eigen::Matrix3d &ATPA, Eigen::Vector3d &ATPb,
											 ...
											 float residual_window_size = 0.1)
		{
			...
			for (int i = 0; i < (*Corr).size(); i++)
			{
				...
				if (t_index != -1)
				{
					...
					if (dist_weight_or_not)
						w = w * get_weight_by_dist(std::sqrt(qx * qx + qy * qy));
		...
		//point-to-line cost (calculated using normal vector)
		...
		//point-to-line cost (calculated using primary direction vector)
											   ...
											   float residual_window_size = 0.1)
		{
			for (int i = 0; i < (*Corr).size(); i++)
			{
				...
				if (t_index != -1)
				{
					...
					// Amat(0, 1) = nxy;
					// Amat(0, 2) = nxz;
					// Amat(0, 3) = -nxy * pz + nxz * py;
					// Amat(0, 4) = nx2 * pz - pz - nxz * px;
					// Amat(0, 5) = -nx2 * py + py + nxy * px;
					// Amat(1, 0) = nxy;
					// Amat(1, 1) = ny2 - 1;
					// Amat(1, 2) = nyz;
					// Amat(1, 3) = -ny2 * pz + pz + nyz * py;
					// Amat(1, 4) = nxy * pz - nyz * px;
					// Amat(1, 5) = ny2 * px - px - nxy * py;
					// Amat(2, 0) = nxz;
					// Amat(2, 1) = nyz;
					// Amat(2, 2) = nz2 - 1;
					// Amat(2, 3) = nz2 * py - py - nyz * pz;
					// Amat(2, 4) = -nz2 * px + px + nxz * pz;
					// Amat(2, 5) = -nxz * py + nxy * pz;

					// bvec(0, 0) = (1 - nx2) * dx - nxy * dy - nxz * dz;
					// bvec(1, 0) = -nxy * dx + (1 - ny2) * dy - nyz * dz;
					// bvec(2, 0) = -nxz * dx - nyz * dy + (1 - nz2) * dz;

					//version 2: //TODO: double-check
					Amat(0, 0) = 0;
					Amat(0, 1) = -nz;
					Amat(0, 2) = ny;
					...
					if (residual_weight_or_not)
					{
						wx = wx * get_weight_by_residual(ed, residual_window_size /* std::sqrt(dist/15.0)*/);
		...
		inline float get_weight_by_dist(float dist, float unit_dist = 60.0, float base_value = 0.7) //TODO: change thge distance weight according to the iteration number (mathematic deducing)
		{
		...
		//TODO: the residual should be something that comes from the transfomation estimated by multiple metrics
								  ...
								  int down_rate = 2)
		{
			CFilter<PointT> cfilter;
			cfilter.random_downsample_pcl(pc_ground_sc, (int)(pc_ground_tc->points.size() / down_rate));
			cfilter.random_downsample_pcl(pc_pillar_sc, (int)(pc_pillar_tc->points.size() / down_rate));
			cfilter.random_downsample_pcl(pc_facade_sc, (int)(pc_facade_tc->points.size() / down_rate));
			cfilter.random_downsample_pcl(pc_beam_sc, (int)(pc_beam_tc->points.size() / down_rate));
			cfilter.random_downsample_pcl(pc_roof_sc, (int)(pc_roof_tc->points.size() / down_rate));
			cfilter.random_downsample_pcl(pc_vertex_sc, (int)(pc_vertex_tc->points.size() / down_rate));
			return true;
		}
	};

	//TODO: add cregistration as a member object
