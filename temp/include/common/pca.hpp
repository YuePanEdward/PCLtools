...
#pragma omp parallel for private(i) //Multi-thread
			for (i = 0; i < in_cloud->points.size(); i++)
			{
...
#pragma omp parallel for private(i) //Multi-thread
			for (i = 0; i < in_cloud->points.size(); i += pca_down_rate)
			{
				...
				tree.radiusSearch(i, neighborhood_r, search_indices, distances, neighborhood_k);
		...
		// R - K neighborhood (with built-kd tree)
		//within the radius, we would select the nearest K points for calculating PCA
		bool get_pc_pca_feature(typename pcl::PointCloud<PointT>::Ptr in_cloud,
		...
		*/
		...
		bool assign_normal(PointT &pt, pca_feature_t &pca_feature, bool is_plane_feature = true)
		{
			if (is_plane_feature)
			{
				...
				pt.normal[3]=pca_feature.planar_2; //planrity    //save in the un-used normal[3]  (PointNormal4D)
			}
			else
			{
				...
				pt.normal[3]=pca_feature.linear_2; //linarity    //save in the un-used normal[3]  (PointNormal4D)
			}
			return true;
		}
