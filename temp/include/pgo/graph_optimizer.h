...
public:
  ...
  bool set_equal_weight(bool on_or_not);

  bool optimize_pose_graph_g2o(cloudblock_Ptrs &all_blocks, constraints &all_cons);

  bool optimize_pose_graph_ceres(cloudblock_Ptrs &all_blocks, constraints &all_cons,
                                 double stable_threshold_tran = 0.5, double stable_threshold_rot = 0.01);
...
private:
  ...
  bool fix_node_ceres(int block_id, float translate_thre, float quat_thre);
  ...
  //commom
  // TODO: the threshold here is not correct
  bool update_optimized_edges(constraints &all_cons, bool fixed_reg_edge = true,
                              float wrong_edge_ratio_thre = 0.1, float wrong_edge_translation_thre = 4.0, float wrong_edge_rotation_thre = 20.0);

  void set_adjacent_edge_information_matrix(Matrix6d &vc_matrix);

  float determine_weight(int node0_type, int node1_type, int edge_type);
